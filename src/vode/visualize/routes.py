...
if FASTAPI_AVAILABLE:
    ...
    async def get_graph(request: Request, max_depth: Optional[int] = None):
        ...
        if not graph_path.exists():
            raise HTTPException(
                status_code=404, detail=f"Graph file not found: {graph_file}"
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error loading graph: {str(e)}"
            )
    ...
    async def get_stats(request: Request):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Error loading stats: {str(e)}"
            )
